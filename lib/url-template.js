...
function readValue (expression, payload, context, errorCode = 'INVALID_URL_TEMPLATE', label = 'URL') {
  ...
  const roots = {
    ...
    context,
    gateway: context,
    // `global` is only populated by callers that pass a context.global
    // accessor (see lib/node-utils.js#globalContextAccessor). It lets
    // Gateway API Config / Gateway Adapter headers read Node-RED's global
    // context, e.g. {{global.apiToken}} for a token refreshed by another
    // flow. context.global is a Proxy: a top-level property read calls
    // global.get(name); nested properties on the returned value are plain
    // object access.
    global: context?.global || {}
  }
  if (!Object.prototype.hasOwnProperty.call(roots, root)) {
    throw templateError(`${label} template root "${root}" is not supported; use payload, request, msg.payload, gateway, or global`, errorCode)
  }
